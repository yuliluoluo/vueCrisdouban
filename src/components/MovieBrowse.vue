...
<div class="browse">
	...
	<div class="list">
		...
		:key="index" tag="li" :to="'/MovieDetail/' + item.id" :data="item.id">
			...
			<p>{{item.title}}</p>
			<!-- <div class="rate">
				<el-rate class="star" disabled v-model="item.rating.value"></el-rate>
				...
				.then(({ data }) => {
					...
					if (data[type]) {
						...
						for(var i = 0; i < newData.length; i++) {
							...
							let r=Math.round(newData[i].rating.value);
							let arr=[0,0,0,0,0];
							let index=0;
							while(r){
								arr[index]=1;
						...
						for(var j = 0; j < newOthers.length; j++) {
							...
							let r=Math.round(newOthers[j].rating.value);
							let arr=[0,0,0,0,0];
							let index=0;
							while(r){
								arr[index]=1;
